...
The ``Base Freq Distance'' algorithm takes a Characters block as input and produces a Distances block as output.

\subsubsection{Binary To Splits}

The ``Binary To Splits'' algorithm takes a Characters block as input and produces a Splits block as output.
...
The ``Characters Filter'' algorithm takes a Characters block as input and produces a Characters block as output.
...
The ``Codominant'' algorithm takes a Characters block as input and produces a Distances block as output.
...
The ``Dice'' algorithm takes a Characters block as input and produces a Distances block as output.
...
The ``DNA To Splits'' algorithm takes a Characters block as input and produces a Splits block as output.
...
The ``Estimate Invariable Sites'' algorithm takes a Characters block as input and produces a Report block as output.
...
The ``External Program'' algorithm takes a Characters block as input and produces a Trees block as output.
...
The ``F81'' algorithm takes a Characters block as input and produces a Distances block as output.
...
The ``F84'' algorithm takes a Characters block as input and produces a Distances block as output.
...
The ``GTR'' algorithm takes a Characters block as input and produces a Distances block as output.
...
The ``Gene Content Distance'' algorithm takes a Characters block as input and produces a Distances block as output.
...
The ``HKY85'' algorithm takes a Characters block as input and produces a Distances block as output.
...
The ``Hamming Distances'' algorithm takes a Characters block as input and produces a Distances block as output.
...
The ``Hamming Distances Ambig States'' algorithm takes a Characters block as input and produces a Distances block as output.
...
The ``Jaccard'' algorithm takes a Characters block as input and produces a Distances block as output.
...
The ``Jukes Cantor'' algorithm takes a Characters block as input and produces a Distances block as output.
...
The ``K2P'' algorithm takes a Characters block as input and produces a Distances block as output.
...
The ``K3ST'' algorithm takes a Characters block as input and produces a Distances block as output.
...
The ``Log Det'' algorithm takes a Characters block as input and produces a Distances block as output.
...
The ``Median Joining'' algorithm takes a Characters block as input and produces a Network block as output.
...
The ``Nei Li Restriction Distance'' algorithm takes a Characters block as input and produces a Distances block as output.
...
The ``Nei Miller'' algorithm takes a Characters block as input and produces a Distances block as output.
...
The ``Parsimony Splits'' algorithm takes a Characters block as input and produces a Splits block as output.
...
The ``Phi Test'' algorithm takes a Characters block as input and produces a Report block as output.
...
The ``Protein M Ldist'' algorithm takes a Characters block as input and produces a Distances block as output.
...
The ``Uncorrected P'' algorithm takes a Characters block as input and produces a Distances block as output.
...
The ``Upholt'' algorithm takes a Characters block as input and produces a Distances block as output.
...
The ``Bio NJ'' algorithm takes a Distances block as input and produces a Trees block as output.
...
The ``Buneman Tree'' algorithm takes a Distances block as input and produces a Splits block as output.

See:  H.-J. Bandelt and A.W.M.Dress. A canonical decomposition theory for metrics on a finite set. Advances in Mathematics, 92:47–105, 1992.

\subsubsection{Delta Score}

The ``Delta Score'' algorithm takes a Distances block as input and produces a Report block as output.

See: B. R. Holland, K. T. Huber, A. Dress, V. Moulton, ? Plots: A Tool for Analyzing Phylogenetic Distance Data, Molecular Biology and Evolution, 19(12):2051–2059, 2002.

\subsubsection{Min Spanning Network}

The ``Min Spanning Network'' algorithm takes a Distances block as input and produces a Network block as output.
...
The ``Min Spanning Tree'' algorithm takes a Distances block as input and produces a Trees block as output.

See:  Excoffier L, Smouse PE. Using allele frequencies and geographic subdivision to reconstruct gene trees within a species: molecular variance parsimony (1994) Genetics.136(1):343-59.

\subsubsection{Neighbor Joining}

The ``Neighbor Joining'' algorithm takes a Distances block as input and produces a Trees block as output.
...
The ``Neighbor Net'' algorithm takes a Distances block as input and produces a Splits block as output.
...
The ``P Co A'' algorithm takes a Distances block as input and produces a Network block as output.
...
The ``Split Decomposition'' algorithm takes a Distances block as input and produces a Splits block as output.

See:  H.-J.Bandelt and A.W.M.Dress. A canonical decomposition theory for metrics on a finite set. Advances in Mathematics, 92:47–105, 1992.

\subsubsection{UPGMA}

The ``UPGMA'' algorithm takes a Distances block as input and produces a Trees block as output.
...
The ``Bootstrap Splits'' algorithm takes a Splits block as input and produces a Splits block as output.
...
The ``Dimension Filter'' algorithm takes a Splits block as input and produces a Splits block as output.
...
The ``Greedy Tree'' algorithm takes a Splits block as input and produces a Trees block as output.

See: D.H. Huson, R. Rupp and C. Scornavacca, Phylogenetic Networks, Cambridge, 2012.

\subsubsection{Phylogenetic Diversity}

The ``Phylogenetic Diversity'' algorithm takes a Splits block as input and produces a Report block as output.
...
The ``Shapley Values'' algorithm takes a Splits block as input and produces a Report block as output.

See: Volkmann L, Martyn I, Moulton V, Spillner A, Mooers AO. Prioritizing populations for conservation using phylogenetic networks. PLoS ONE 9(2):e88945 (2014)

\subsubsection{Show Splits}

The ``Show Splits'' algorithm takes a Splits block as input and produces a View block as output.
...
The ``Splits Filter'' algorithm takes a Splits block as input and produces a Splits block as output.
...
The ``Weights Slider'' algorithm takes a Splits block as input and produces a Splits block as output.
...
The ``ALTS External'' algorithm takes a Trees block as input and produces a Trees block as output.
...
The ``ALTS Network'' algorithm takes a Trees block as input and produces a Trees block as output.
...
The ``Autumn Algorithm'' algorithm takes a Trees block as input and produces a Trees block as output.
...
The ``Average Consensus'' algorithm takes a Trees block as input and produces a Splits block as output.
...
The ``Average Distances'' algorithm takes a Trees block as input and produces a Distances block as output.

See: Francois-Joseph Lapointe, Guy Cucumel, The Average Consensus Procedure: Combination of Weighted Trees Containing Identical or Overlapping Sets of Taxa. Systematic Biology, 46(2):306-312 (1997).

\subsubsection{Bootstrap Tree}

The ``Bootstrap Tree'' algorithm takes a Trees block as input and produces a Trees block as output.
...
The ``Bootstrap Tree Splits'' algorithm takes a Trees block as input and produces a Splits block as output.
...
The ``Cluster Network'' algorithm takes a Trees block as input and produces a Trees block as output.
...
The ``Consensus Network'' algorithm takes a Trees block as input and produces a Splits block as output.
...
The ``Consensus Outline'' algorithm takes a Trees block as input and produces a Splits block as output.
...
The ``Consensus Splits'' algorithm takes a Trees block as input and produces a Splits block as output.
...
The ``Consensus Tree'' algorithm takes a Trees block as input and produces a Trees block as output.
...
The ``Enumerate Contained Trees'' algorithm takes a Trees block as input and produces a Trees block as output.
...
The ``Filtered Super Network'' algorithm takes a Trees block as input and produces a Splits block as output.
...
The ``List One RSPR Trees'' algorithm takes a Trees block as input and produces a Report block as output.
...
The ``Loose And Lacy'' algorithm takes a Trees block as input and produces a Trees block as output.
...
The ``Phylogenetic Diversity'' algorithm takes a Trees block as input and produces a Report block as output.
...
The ``Reroot Or Reorder Trees'' algorithm takes a Trees block as input and produces a Trees block as output.
...
The ``Rooted Consensus Tree'' algorithm takes a Trees block as input and produces a Trees block as output.
...
The ``Show Trees'' algorithm takes a Trees block as input and produces a View block as output.
...
The ``Super Network'' algorithm takes a Trees block as input and produces a Splits block as output.
...
The ``Tree Diversity Index'' algorithm takes a Trees block as input and produces a Report block as output.
...
The ``Tree Selector'' algorithm takes a Trees block as input and produces a Trees block as output.
...
The ``Tree Selector Splits'' algorithm takes a Trees block as input and produces a Splits block as output.
...
The ``Trees Filter'' algorithm takes a Trees block as input and produces a Trees block as output.

\subsubsection{Trees Filter 2}

The ``Trees Filter 2'' algorithm takes a Trees block as input and produces a Trees block as output.
...
The ``Unrooted Shapley Values'' algorithm takes a Trees block as input and produces a Report block as output.
...
The ``Show Network'' algorithm takes a Network block as input and produces a View block as output.
