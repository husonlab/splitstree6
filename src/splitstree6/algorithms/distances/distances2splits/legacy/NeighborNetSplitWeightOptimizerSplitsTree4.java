/*
 * NeighborNetSplitWeightOptimizer.java Copyright (C) 2022 Daniel H. Huson
 *
...
public class NeighborNetSplitWeightOptimizerSplitsTree4 {
    ...
    static public ArrayList<ASplit> getAllSplits(int ntax, int[] ordering, double weight) {
        ...
        for (int i = 0; i < ntax; i++) {
            ...
            for (int j = i + 1; j < ntax; j++) {
                t.set(ordering[j + 1]);
                splits.add(new ASplit(t, ntax, weight));
            }
        }
        return splits;
    }
    ...
    static public ArrayList<ASplit> computeWeightedSplits(int[] ordering, DistancesBlock dist, Options options) {
        ...
        if (ntax == 1)
            return splits;
        if (ntax == 2) {
            var split = new ASplit(BitSetUtils.asBitSet(1), BitSetUtils.asBitSet(2), dist.get(1, 2));
            splits.add(split);
            return splits;
        }
        ...
        for (int i = 0; i < ntax; i++) {
            ...
            for (int j = i + 1; j < ntax; j++) {
                ...
                if (x[index] > options.cutoff)
                    splits.add(new ASplit(t, ntax, x[index]));
                index++;
...
//            { return computeWeightedSplits(ordering,dist,var,options);}
    ...
    static private void runActiveConjugate(int ntax, double[] d, double[] W, double[] x, Options options) {
        ...
        boolean computeRegularised = (options.regularization != Options.Regularization.NNLS);
        double lambda = 0.0;
