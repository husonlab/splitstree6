/*
 *  Splits2ReportBase.java Copyright (C) 2024 Daniel H. Huson
 *
...
abstract public class Splits2ReportBase extends Algorithm<SplitsBlock, ReportBlock> {
	 ...
	 * @param splitsBlock  current splits
	 * @param selectedTaxa selected taxa, if required
	 * @return text to present
	 */
	abstract String runAnalysis(ProgressListener progress, TaxaBlock taxaBlock, SplitsBlock splitsBlock, Collection<Taxon> selectedTaxa);
	...
	public boolean isApplicable(TaxaBlock taxa, SplitsBlock datablock) {
		return taxa.getNtax() > 0 && datablock.getNsplits() > 0;
	}
}
