 ...
 */
public class NexusWriter extends TriatsWriterBase {
	...
	public void write(Writer w, TaxaBlock taxaBlock, TraitsBlock traitsBlock) throws IOException {
		if (isOptionPrependNexus())
			w.write("#nexus\n");
		var output = new TraitsNexusOutput();
		output.setTitleAndLink(getTitle(), getLink());
		if (asWorkflowOnly)
			output.write(w, new TaxaBlock(), new TraitsBlock());
		else
			output.write(w, taxaBlock, traitsBlock);
		w.flush();
	}
