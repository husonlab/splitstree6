/*
 * NeiMiller.java Copyright (C) 2024 Daniel H. Huson
 *
			...
			for (int j = i + 1; j <= ntax; j++) {
				for (int k = 1; k <= num_classes; k++) {
					...
					if (bot != 0)
						...
						if (!warned_sij) {
							System.err.println("NeiMiller: denominator zero in equation (2)");
							warned_sij = true;
						}
						sij_k[i][j][k] = 100000;
					}
			...
			for (int j = i + 1; j <= ntax; j++) {
				for (int k = 1; k <= num_classes; k++) {
					if (class_value[k] == 0) {
						dhij_k[i][j][k] = 100000;
						if (!warned_dhij) {
							System.err.println("NeiMiller: denominator zero in equation (3)");
							warned_dhij = true;
						}
					} else
						dhij_k[i][j][k]
		...
		for (int i = 1; i <= ntax; i++) {
			for (int j = i + 1; j <= ntax; j++) {
				...
				if (bottom != 0)
					...
					if (!warned_dist) {
						System.err.println("NeiMiller: denominator zero in equation (4)");
						warned_dist = true;
					}
