/*
 *  TreeFilterTabPresenter.java Copyright (C) 2024 Daniel H. Huson
 *
	...
	public TreeFilterTabPresenter(MainWindow mainWindow, TreeFilterTab treeFilterTab, AlgorithmNode<TreesBlock, TreesBlock> treesFilterNode, TreesFilter treesFilter) {
		...
		Runnable updateTable = () -> {
			...
			if (parentNode != null) {
				...
				if (treesFilter.getOptionDisabledTrees() != null) {
					disabled.addAll(Arrays.stream(treesFilter.getOptionDisabledTrees()).filter(Objects::nonNull).toList());
				}
