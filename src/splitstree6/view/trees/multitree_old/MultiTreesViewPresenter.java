...
public class MultiTreesViewPresenter implements IDisplayTabPresenter {
	...
	public MultiTreesViewPresenter(MainWindow mainWindow, MultiTreesView multiTreesView, ObservableList<PhyloTree> phyloTrees) {
		...
		{
			multiTreesView.pageNumberProperty().addListener((v, o, n) -> controller.getPagination().setCurrentPageIndex(n.intValue() - 1));
			controller.getPagination().currentPageIndexProperty().addListener((v, o, n) -> multiTreesView.setPageNumber(n.intValue() + 1));
		...
		{
			controller.getFirstPageButton().setOnAction(e -> multiTreesView.setPageNumber(1));
			controller.getFirstPageButton().disableProperty().bind(multiTreesView.isEmptyProperty().or(multiTreesView.pageNumberProperty().lessThanOrEqualTo(1)));

			controller.getPreviousPageButton().setOnAction(e -> multiTreesView.setPageNumber(multiTreesView.getPageNumber() - 1));
			controller.getPreviousPageButton().disableProperty().bind(controller.getFirstPageButton().disableProperty());

			multiTreesView.pageNumberProperty().addListener((v, o, n) -> controller.getPageTextField().setText(n.toString()));
			controller.getPageTextField().setTextFormatter(new TextFormatter<>(new IntegerStringConverter()));
			controller.getPageTextField().setText(String.valueOf(multiTreesView.getPageNumber()));
			controller.getPageTextField().setOnAction(e -> {
				if (NumberUtils.isInteger(controller.getPageTextField().getText())) {
					...
					multiTreesView.setPageNumber(page);
				}
			});
			multiTreesView.pageNumberProperty().addListener((v, o, n) -> {
				if (n.intValue() < 1)
					Platform.runLater(() -> multiTreesView.setPageNumber(1));
				else if (n.intValue() >= numberOfPages.get())
					Platform.runLater(() -> multiTreesView.setPageNumber((Math.max(1, numberOfPages.get()))));
			});

			controller.getPageTextField().disableProperty().bind(Bindings.size(phyloTrees).lessThanOrEqualTo(1));

			controller.getNextPageButton().setOnAction(e -> multiTreesView.setPageNumber(multiTreesView.getPageNumber() + 1));
			controller.getNextPageButton().disableProperty().bind(multiTreesView.isEmptyProperty().or(multiTreesView.pageNumberProperty().greaterThanOrEqualTo(numberOfPages)));

			controller.getLastPage().setOnAction(e -> multiTreesView.setPageNumber(phyloTrees.size()));
			...
			controller.getUpdateButton().disableProperty().bind(Bindings.size(phyloTrees).isEqualTo(0));
		}
