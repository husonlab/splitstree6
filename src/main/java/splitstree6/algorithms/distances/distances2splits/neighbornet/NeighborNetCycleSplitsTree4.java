/*
 *  NeighborNetCycleSplitsTree4.java Copyright (C) 2024 Daniel H. Huson
 *
...
public class NeighborNetCycleSplitsTree4 {
	...
	public static int[] compute(int nTax, double[][] dist) {
		...
		final Stack<NetNode> joins = joinNodes(mat, nodesHeader, nTax);
		// System.err.println("Ordering: "+ Basic.toString(ordering));

		return expandNodes(nTax, joins, nodesHeader);
	}
	 ...
	 */
	static private Stack<NetNode> joinNodes(double[][] D, NetNode nodesHead, int num_nodes) {
...
/* Replace x2,x,y,y2 by with two vertices... performed using two
	 ...
	 * @param nodesHead the net nodes
	 */
	static private int[] expandNodes(int nTax, Stack<NetNode> joins, NetNode nodesHead) {
