 ...
 * <p>
 * Assumptions about host Graph API (adapt or shim to your classes):
 * - Graph#nodes(): Collection<Node>
 * - Graph#edges(): Collection<Edge>
 * - Graph#getAdjacentNodes(Node v): Collection<Node>
 * - Graph#getAdjacentEdges(Node v): Collection<Edge>
...
public class ParsimonyLabeler<Node, Edge> {
	private final GraphAccessor<Node, Edge> G;

	public ParsimonyLabeler(GraphAccessor<Node, Edge> accessor) {
		this.G = accessor;
	}
	...
	public Map<Node, String> labelAllSites(Map<Node, String> knownSeqs) {
		...
		for (int j = 0; j < L; j++) {
			...
			if (sigma.size() <= 1) {
				...
				labels = new HashMap<>();
				for (Node v : G.nodes()) labels.put(v, a);
		...
		for (Node v : G.nodes()) agg.put(v, new StringBuilder(L));
		for (int j = 0; j < L; j++) {
			Map<Node, Character> lab = siteLabels.get(j);
			for (Node v : G.nodes()) agg.get(v).append(lab.get(v));
		}
	...
	private Map<Node, Character> solveBinarySite(Map<Node, Character> terminals) {
		...
		for (Node v : G.nodes()) FN.addNode(v);
		FN.addSourceSink();
		...
		for (Edge e : G.edges()) {
			...
			FN.addUndirectedCapacity(u, v, 1.0);
		}

		FN.maxFlow();
		Set<Node> Sside = FN.minCutSourceSide();
		Map<Node, Character> lab = new HashMap<>();
		for (Node v : G.nodes()) lab.put(v, Sside.contains(v) ? sA : sB);
		return lab;
	}
	...
	// --- Host graph accessors (inject to adapt to your classes) ---

	public interface GraphAccessor<Node, Edge> {
		Iterable<Node> nodes();
