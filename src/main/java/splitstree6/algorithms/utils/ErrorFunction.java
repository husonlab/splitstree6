/*
 *  ErrorFunction.java Copyright (C) 2024 Daniel H. Huson
 *
...
/**
 * Code for computing the error function and complimentary error function.
 * Dave Bryant, 2005
 */
...
// eta0 = .1*d1mach(3)
		...
		nterf = ErrorFunction.initds(erfcs, 21, eta0);
...
// ERF(X) = 1.0 - ERFC(X)  FOR  -1.0 .LE. X .LE. 1.0
			...
			if (y <= sqeps) {
				...
				ans = x * (1.0 + ErrorFunction.dcsevl(2.0 * x * x - 1.0, erfcs, nterf));
...
// ERF(X) = 1.0 - ERFC(X) FOR ABS(X) .GT. 1.0
		...
		if (y <= xbig) {
			...
			ans = ErrorFunction.sign(1.0 - ErrorFunction.derfc(y), x);

		} else {

			ans = ErrorFunction.sign(1.0, x);
...
// eta0 = .1*d1mach(3)
		...
		nterf = ErrorFunction.initds(erfcs, 21, eta0);
		nterfc = ErrorFunction.initds(erfccs, 59, eta0);
		nterc2 = ErrorFunction.initds(erc2cs, 49, eta0);
...
// ERFC(X) = 1.0 - ERF(X)  FOR ABS(X) .LE. 1.0
			...
			if (y < sqeps) {
				...
				ans = 1.0 - x * (1.0 + ErrorFunction.dcsevl(2.0 * x * x - 1.0, erfcs, nterf));
...
// ERFC(X) = 1.0 - ERF(X)  FOR  1.0 .LT. ABS(X) .LE. XMAX
		...
		if (y <= 4.0) {
			...
			ans = (Math.exp(-y) / Math.abs(x)) * (0.5 +
												  ErrorFunction.dcsevl((8.0 / y - 5.0) / 3.0, erc2cs, nterc2));

		} else {

			ans = (Math.exp(-y) / Math.abs(x)) * (0.5 +
												  ErrorFunction.dcsevl(8.0 / y - 1.0, erfccs, nterfc));
...
C                 Numerical AnalysisMethod, Oxford University Press, 1968,
C                 page 56.
