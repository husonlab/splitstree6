...
public class WorkflowSetup {
	...
	public static Workflow apply(String fileName, Workflow workflow) {
		...
		if (clazz.equals(CharactersBlock.class)) {
			workflow.setupTopAndWorkingNodes(sourceBlock, new CharactersLoader(), new TaxaBlock(), new CharactersBlock());
			...
			workflow.setupTopAndWorkingNodes(sourceBlock, new DistancesLoader(), new TaxaBlock(), new DistancesBlock());
			var distancesNode = workflow.getTopDataNode();
			var splitsNode = workflow.newDataNode(new SplitsBlock());
			workflow.newAlgorithmNode(new NeighborNet(), workflow.getWorkingTaxaNode(), distancesNode, splitsNode);
			workflow.newAlgorithmNode(new ShowSplitsConsole(), workflow.getWorkingTaxaNode(), splitsNode, workflow.newDataNode(new SinkBlock()));
			// todo: replace by calculation of network
		} else if (clazz.equals(SplitsBlock.class)) {
			workflow.setupTopAndWorkingNodes(sourceBlock, new SplitsLoader(), new TaxaBlock(), new SplitsBlock());
			var splitsNode = workflow.getTopDataNode();
			workflow.newAlgorithmNode(new ShowSplitsConsole(), workflow.getWorkingTaxaNode(), splitsNode, workflow.newDataNode(new SinkBlock()));
			// todo: replace by calculation of network
		} else if (clazz.equals(TreesBlock.class)) {
			workflow.setupTopAndWorkingNodes(sourceBlock, new TreesLoader(), new TaxaBlock(), new TreesBlock());
			var splitsNode = workflow.newDataNode(new SplitsBlock());
			workflow.newAlgorithmNode(new ConsensusNetwork(), workflow.getWorkingTaxaNode(), workflow.getTopDataNode(), splitsNode);
			workflow.newAlgorithmNode(new ShowSplitsConsole(), workflow.getWorkingTaxaNode(), splitsNode, workflow.newDataNode(new SinkBlock()));
			// todo: replace by calculation of network
		}
		/*
		else if (clazz.equals(NetworkBlock.class)) {
			workflow.setupTopAndWorkingNodes(sourceBlock,new NetworkLoader(),new TaxaBlock(), new NetworksBlock());
