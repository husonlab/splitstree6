/*
 *  NeighborNetCycle2023.java Copyright (C) 2024 Daniel H. Huson
 *
...
public class NeighborNetCycle2023 {
	...
	public static int[] computeOrdering(DistancesBlock distancesBlock) {
		...
		while (components.size() >= 2) {
			...
			if (P.size() == 1 && Q.size() == 1) {
				...
				var p = pq.getFirst();
				var pb = P.other(p); // \bar p in text
				var q = pq.getSecond();
				var qb = Q.other(q); // \bar q in text
				...
				for (var i = 0; i < components.size(); i++) {
					if (i != ip && i != iq) {
						for (var r : components.get(i).values()) {
							if (r != p && r != q && r != pb && r != qb) { // this 'if' should be superfluous
		...
		if (components.size() == 2) {
			...
			for (var ip = 0; ip < components.size(); ip++) {
				...
				for (var iq = ip + 1; iq < components.size(); iq++) {
					...
					var sumP = 0.0;
					var sumQ = 0.0;
					for (var is = 0; is < components.size(); is++) {
						if (is != ip && is != iq) {
							var S = components.get(is);
							sumP += averageD(D, P, S);
							sumQ += averageD(D, Q, S);
						}
					}

					var adjustedD = (components.size() - 2) * averageD(D, P, Q) - sumP - sumQ;
	...
	private static Pair<Integer, Integer> selectClosest2vs2(int ip, int iq, double[][] D, ArrayList<Component> components) {
		...
		var p1R = D[p1][q1] + D[p1][q2];
		var p2R = D[p2][q1] + D[p2][q2];
		var q1R = D[p1][q1] + D[p2][q1];
		var q2R = D[p1][q2] + D[p2][q2];
	...
	private record Component(int first, int second) {
