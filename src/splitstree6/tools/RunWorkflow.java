...
public class RunWorkflow extends Application {
	...
	private void run(String[] args) {
		try {
			...
			var inputFilesFinal = inputFiles;
			var outputFilesFinal = outputFiles;
			...
			for (var i = 0; i < inputFilesFinal.length; i++) {
				final var inputFile = inputFilesFinal[i];
				System.err.println("++++ Processing " + inputFile + " (" + (i + 1) + " of " + inputFilesFinal.length + ") ++++");
				...
				try {
					final var outputFile = (outputFilesFinal.length == inputFilesFinal.length ? outputFilesFinal[i] : outputFilesFinal[0]);
